...
def runBitCodeParserTestCases(args=None):
...
def getLLVMVersion(llvmProgram):
    ...
    if printLLVMVersion is None:
        exit("Could not find the LLVM version 3.x using command '" + llvmProgram + " --version'.\nYou may also use the LLVM binaries from " + _toolDir +"tools/llvm/bin" )
    else:
...
mx.update_commands(_suite, {
    ...
    'su-tests-bcparser' : [runBitCodeParserTestCases, ''],
